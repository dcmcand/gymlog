...
|---|------|-------|--------------|----------|
| 1 | Open Settings from a gear on the Calendar tab; it shows the app version | Tapping the gear on Calendar opens Settings showing "GymLog 1.5" (v1.5 build) | narrated: on the phone, tap gear; capture `adb exec-out screencap` screenshot of Settings || 2026-09-25, Pixel 9a, v1.5 debug: Calendar gear (content-desc "Settings", bounds [975,357][1038,420]) tapped via adb; resulting screen text: "Settings", "Backup", "Export data", "Import data", "GymLog 1.5" (screenshot reviewed). PASS |
| 2 | Export all data to a JSON file I choose, readable in a text editor | Export writes `gymlog-backup-<date>.json` to the chosen location (Downloads in the check); the pulled file is pretty-printed JSON with `formatVersion: 1` and table counts equal to the device DB's row counts | narrated: export on phone; `adb pull` the file; `jq` key/counts vs `sqlite3` `SELECT COUNT(*)` per table on the pulled DB || Exported gymlog-backup-2026-09-25.json (172028 B) to Downloads; pulled file is pretty-printed JSON, `formatVersion: 1`, `appVersion: "1.5"`. jq counts vs device DB: exercises 18/18, workouts 5/5, workoutExercises 19/19, sessions 35/35, sets 527/527. PASS |
| 3 | Importing into a fresh install restores everything: exercises, workouts, sessions, sets, settings, history and progress charts | After import into a fresh install, every table's rows are identical to the source DB; history and a progress chart show the same data | automated: `BackupCodecTest` round trip + golden file; narrated: `sqlite3` dump of all 5 tables from source and destination DBs, `diff` is empty; screenshot of one progress chart on both installs || Automated: BackupCodecTest round trip + golden file pass (suite green). Narrated: imported the export into a separate fresh install (throwaway `com.gymlog.app.verify`, see Task 5 ruling); sqlite3 dump of all 5 tables (614 lines) source vs destination: `diff` empty. bench press progress screen identical on both installs ("Sessions: 9", "Last: 47.5 kg (May 11)", "Personal Best: 52.5 kg (Apr 17)"; screenshots byte-size equal 62281/62281). PASS |
| 4 | Import asks before replacing; cancelling changes nothing | Picking a valid file shows the confirmation with both workout counts; Cancel leaves every table dump identical | narrated: screenshot of dialog; `sqlite3` table dump before and after Cancel, `diff` empty || Dialog text on fresh install: "Replace all data?" / "This device's 0 workouts will be replaced by the file's 35 workouts. This can't be undone." / Cancel / Replace. After Cancel, table dump before vs after: `diff` empty. PASS |
| 5 | A bad file shows a clear error and changes nothing | Importing (a) a non-JSON file, (b) a backup with `formatVersion: 99`, (c) a backup with a set pointing at a missing session each shows its specific message; DB unchanged | automated: `BackupCodecTest` rejection table; narrated: push the three files, import each, screenshot each snackbar, `sqlite3` dump unchanged || Automated: BackupCodecTest rejection table passes (and fails with validation disabled). Narrated: imported three pushed files; snackbars (screenshots reviewed): bad-notjson.json -> "That file isn't a GymLog backup."; bad-newer.json (formatVersion 99) -> "This backup was made by a newer GymLog. Update GymLog, then try again."; bad-corrupt.json (sets[0].sessionId=999999) -> "This backup is damaged and can't be imported."; dump before vs after all three: `diff` empty. PASS |
| 6 | Cannot import during an in-progress workout, and I am told why | With a workout in progress, Import shows "Finish or discard your current workout first"; DB unchanged | automated: `BackupServiceTest` in-progress case; narrated: screenshot of the message || Automated: BackupServiceTest in-progress cases pass. Narrated: started Workout A (session 146 IN_PROGRESS, set 1 EASY), Import -> picked backup -> snackbar "Finish or discard your current workout first."; dump before vs after: `diff` empty. PASS |
| 7 | A workout in progress at export time can be resumed after import | Export with an in-progress session, import into a fresh install, Calendar offers Resume and the active workout shows the same completed/pending sets | narrated: screenshots before export and after import/resume || Exported with session 146 IN_PROGRESS (file: 1 EASY + 12 PENDING sets for 146); `pm clear` on the throwaway app (0 rows), imported (dialog: 0 -> 36 workouts), Calendar showed "Workout in progress - tap to resume"; resumed screen shows bench press set 1 completed (green, 8) and all others pending; all 5 tables identical to pre-export (`diff` empty). PASS |
| 8 | Renamed app installs alongside the old one and data moves across via export/import | `adb shell pm list packages` shows both `com.gymlog.app` and `io.github.dcmcand.gymlog`; after importing the 1.5 export into 2.0, all 5 tables diff clean between the two apps' DBs | narrated: `pm list packages` output; `sqlite3` dumps from both apps and empty `diff` || 2026-09-25, Pixel 9a: `pm list packages` showed `package:io.github.dcmcand.gymlog` and `package:com.gymlog.app` side by side. Imported the 1.5 export into 2.0 (dialog: 0 -> 35 workouts); sqlite3 dump of all 5 tables from both apps (614 lines each): `diff` empty. User reviewed history in the new app, then old app uninstalled (its DB and the export kept in ~/gymlog-backups). PASS |
| 9 | Watchapp works with the renamed app (open mid-workout, Easy/Hard, skip, extend) | With only `io.github.dcmcand.gymlog` handling the watch, logcat shows context pushed on watchapp open, `10=1`/`10=2` completions recorded in the new app's DB, `10=4` cycling, `10=3` extending rest | narrated: decoded logcat transcript (same decoder used for 1.4) + `sqlite3` row showing the watch-completed set || Only io.github.dcmcand.gymlog installed, watchapp 2.0.0 (companionApp lists both packages). Decoded logcat: workout session 147 started 17:54:22, watchapp START 17:54:29.181, phone pushed 'bench press / 42.5kg x 8 / Set 1 of 5' at 17:54:29.222; `10=2` (Hard) -> new app DB row 1871 bench press set 1 42.5 kg status HARD; earlier run: `10=1` x4 advanced sets, `10=4` cycled bench press -> Romanian deadlift and back to the skipped exercise when done, `10=3` extended rest 83 -> 173 s; 0 FATAL EXCEPTION. PASS |
| 10 | Rest timer notification opens the active workout in the renamed app | Completing a set starts the timer notification; tapping it opens the renamed app's active workout screen | narrated: screenshot of notification and of the screen it opens; `dumpsys activity` top activity shows `io.github.dcmcand.gymlog` || logcat 17:50:37.312: `START u0 {flg=0x20000000 cmp=io.github.dcmcand.gymlog/com.gymlog.app.MainActivity (has extras)}` from uid 10391 (io.github.dcmcand.gymlog PendingIntent) with realCallingUid 10238 (com.android.systemui, the notification shade); the app re-pushed the active workout to the watch 90 ms later; user confirmed the notification opened the active workout. PASS |
| 11 | No new permissions; still no internet | Merged manifest permissions for v1.5 and v2.0 equal 1.4's; no `INTERNET` | automated: existing `ManifestPermissionsTest`; narrated: `aapt2 dump permissions` on the 1.4, 1.5 and 2.0 APKs, compared || v1.5 part: `aapt2 dump permissions` v1.4 release APK vs v1.5 debug APK: identical 7 lines (FOREGROUND_SERVICE, FOREGROUND_SERVICE_SPECIAL_USE, POST_NOTIFICATIONS, POST_PROMOTED_NOTIFICATIONS, VIBRATE, DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION x2), no INTERNET; ManifestPermissionsTest passes. v2.0 part: 2.0 debug APK lists the same 7 permissions as 1.4 and 1.5 after normalizing the app-id-named DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION (`diff` empty), no INTERNET. PASS |
